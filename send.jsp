...
%>
			<tr class="card" style="cursor:pointer;">
				...
				<td class="reciver" onclick="window.location='vese?messid=<%= rs.getInt("messid") %>'"><%=rs.getString("to1")%></td>
				<td onclick="window.location='vese?messid=<%= rs.getInt("messid") %>'"><%=rs.getString("subject")%></td>
				<td onclick="window.location='vese?messid=<%= rs.getInt("messid") %>'"><%= messg.length()>20 ? messg.substring(0,20)+"....":messg%></td>
				
				<td onclick="window.location='vese?messid=<%= rs.getInt("messid") %>'"><%= displayDate %></td>
  ...
  <script>
	...
	function searchMails(){
		...
		for(let i=0;i< rows.length;i++){
			let sender = rows[i].getElementsByClassName("reciver")[0].innerText.toLowerCase();
